 ...
 const heroSlide = `<div class="step-slide hero-slide">${heroImg}<div class="hero-slide-content">${pills}<h1>${escapeAttr(r.title)}</h1>${r.intro ? `<div class="routine-intro">${escapeAttr(r.intro)}</div>` : ''}</div></div>`;
  ...
  const stepSlides = (r.steps || []).map(s => {
    ...
    const btnHTML = s.productUrl
      ? `<a href="${escapeAttr(s.productUrl)}" target="_blank" rel="nofollow noopener" class="step-slide-amazon">Buy on Amazon</a>`
      : '';
return `<div class="step-slide"><div class="step-slide-img-col"><div class="step-slide-img-wrap">${imgHTML}</div></div><div class="step-slide-content"><div class="step-slide-label">STEP ${escapeAttr(s.stepNumber)} - ${escapeAttr(s.title)}</div><h2 class="step-slide-title">${escapeAttr(s.productName)}</h2><div class="step-slide-why">${escapeAttr(s.whyThisStep)}</div>${tipHTML}${waitHTML}${btnHTML}</div></div>`;
  }).join('');
